...
private:
	void flush_day() {
		...
		auto batch = arrow::RecordBatch::Make(schema, symbol_arr->length(), {
			symbol_arr, time_arr, price_arr, size_arr,
			is_bid_arr, is_trade_arr, is_ask_arr, remove_level_arr
		});

		auto filepath = (fs::path(dir) / "irts" / (today + ".parquet")).string();
		auto out = detail::unwrap(arrow::io::FileOutputStream::Open(filepath));
		...
		auto writer = detail::unwrap(parquet::arrow::FileWriter::Open(
			*schema, pool, out, props));

		detail::check(writer->WriteRecordBatch(*batch));
		detail::check(writer->Close());
	}
